...
Date Created: 08/26/2019
     Credits: 
Last Revised: 11/28/2023 (J Newton)
************************************************************************
...
ForEach ($ComputerName in ($computer))
        ...
        {
        Write-Host "$($ComputerName) " -F Yellow -NoNewline
...
Get-WinEvent -ComputerName $ComputerName -FilterHash @{LogName='system';providername='Microsoft-Windows-Winlogon'} -MaxEvents $maxResults|
select $UserProperty,$TypeProperty,$TimeProeprty,@{n="Computer";e={$ComputerName}}
        }
